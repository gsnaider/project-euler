...
public class PrimeUtilTest {
	...
	private static final ImmutableList<Long> primes = ImmutableList.of(2L, 3L, 5L, 7L, 11L, 13L, 17L, 19L, 23L, 29L);

	private static final ImmutableList<Long> nonPrimes = ImmutableList.of(0L, 1L, 4L, 6L, 8L, 9L, 10L, 12L, 14L, 15L, 16L);
	...
	@Test
	public void testIsPrimeWithKnownPrimes_trueForPrimes() {
		for (Long prime : primes) {
			assertThat(PrimeUtil.isPrime(prime, primes.subList(0, primes.indexOf(prime))));
		}
	}

	@Test
	public void testIsPrimeWithKnownPrimes_falseForNonPrimes() {
		for (Long nonPrime : nonPrimes) {
			assertThat(PrimeUtil.isPrime(nonPrime, primes.subList(0, -Collections.binarySearch(primes, nonPrime) - 1)));
		}
	}
